...
/**
 * Fetches gas station data from Firebase and passes it to a callback function.
 * @param {function} callback - A function to handle the retrieved gas station data.
 */
export function getGasStations(callback) {
  // Create a reference to the "gasStations" node in the database
  const gasStationsRef = ref(database, "gasStations");

  // Listen for changes in the "gasStations" node
  onValue(gasStationsRef, (snapshot) => {
